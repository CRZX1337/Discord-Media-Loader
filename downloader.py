...
def download_media(url: str, format_type: str) -> str:
    ...
    # Echter Videotitel + Video ID kombiniert, um Überschreibungen bei denselben Titeln zu vermeiden
    filepath_prefix = os.path.join(temp_dir, '%(title)s_%(id)s')
    ...
    try:
        ...
        # Datei suchen über die im Pattern vergebene ID
        files = glob.glob(os.path.join(temp_dir, f"*_{video_id}.*"))
        ...
        try:
            val = locals().get('info', {}).get('id')
            if val:
                for tmp_file in glob.glob(os.path.join(temp_dir, f"*_{val}.*")):
                    os.remove(tmp_file)
        except Exception:
            pass
        raise e
